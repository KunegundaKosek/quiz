import { answer3 } from './answers3.js';
import Timer from './timer.js';

export default function Test3() {
    const buttonAnswer3 = document.querySelector('#answer3');
    ...
    buttonAnswer3.addEventListener('click', () => {
        ...
        h1.remove();
        buttonAnswer3.remove();

        let score = 0;

        answer3.forEach((item) => {
        ...
        const timer = document.querySelector('#timer');
            ...
            scoreText.textContent = `Liczba punktów: ${score} / 2`;
            scoreText.className = 'main__scoreText';

            const percent = document.createElement('p');
            percent.textContent = `${((score / 2) * 100).toFixed(2)}%`;
